...
const Main = () => {
  ...
  const getProductsByPrice = () => {
    axios.get(`http://localhost:5000/products/search_2?asc=${priceFilterFrom}&&desc=${priceFilterTo}`)
    .then((result)=>{
    ...
    <>
      <div className="conteainer-main-page">
        ...
        <div className="filter-div">
          ...
          <input className="filter-input" type="number" placeholder="from $" onChange={(e)=>{setPriceFilterFrom(e.target.value)}}></input>
          <input className="filter-input" type="number" placeholder="to $" onChange={(e)=>{setPriceFilterTo(e.target.value)}}></input>
          <button className="filter-button" onClick={()=>{getProductsByPrice(); history("/products/search_1")}}>Go</button>
          </div>
          <hr></hr>
          <h5 className="filter-text"><FcFilledFilter size={25}/> Filter Products By Categories</h5>
          <hr></hr>
          <Link to="" className="filter-link">Appliances</Link>
          <Link to="" className="filter-link">Apps & Games</Link>
          <Link to="" className="filter-link">Arts, Crafts, & Sewing</Link>
          <Link to="" className="filter-link">Automotive Parts & Accessories</Link>
          <Link to="" className="filter-link">Baby</Link>
          <Link to="" className="filter-link">Beauty & Personal Care</Link>
          <Link to="" className="filter-link">Clothing, Shoes and Jewelry</Link>
          <Link to="" className="filter-link">Computers</Link>
          <Link to="" className="filter-link">Electronics</Link>
        </div>
